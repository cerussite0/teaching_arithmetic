"""
This file is used for experiments in excluding numbers or digits
"""
...
from main_utils import evaluate_text, evaluate_addition_new, get_encode_decode, evaluate_addition_multidigit
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if eval_addition:
            ...
            test_accuracy, _ = evaluate_addition_new(config, model, ctx, encode, decode, verbose=True, num_digit=num_digit, zero_pad=zero_pad, reverse_ab=reverse_ab, reverse_c=reverse_c, algo_reason=algo_reason, binary=binary)
        if eval_addition_527:
            config['start'] = start_527
            test_accuracy_527, _ = evaluate_addition_new(config, model, ctx, encode, decode, verbose=True, num_digit=num_digit, zero_pad=zero_pad, reverse_ab=reverse_ab, reverse_c=reverse_c, algo_reason=algo_reason, binary=binary)
        print(f"step {iter_num}: train loss {losses['train']:.4f}, val loss {losses['val']:.4f}")
        print(f"step {iter_num}: test acc {test_accuracy:.4f}, 527 test acc {test_accuracy_527:.4f}")
        if wandb_log:
            wandb_dict = {
                "iter": iter_num,
                ...
                "test/accuracy_527": test_accuracy_527 if eval_addition_527 else None
            }
